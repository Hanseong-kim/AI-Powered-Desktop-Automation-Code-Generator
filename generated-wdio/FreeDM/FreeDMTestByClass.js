...
class FreeDMPageByClass {
    ...
    async click2() {
        const c = await getCenterSimple('//*[@ClassName="DownloadsViewItem_V2_QMLTYPE_362"]');
    ...
    async click4() {
        const c = await getCenterSimple('//*[@ClassName="BaseContextMenuItem_QMLTYPE_33" and @Name="링크 복사"]');
        if (c) osClick(c.x, c.y);
        else osClick(2921, -152);
    }

    async click5() {
        const c = await getCenterSimple('//*[@ClassName="BaseLabel_QMLTYPE_9"]');
        if (c) osClick(c.x, c.y);
