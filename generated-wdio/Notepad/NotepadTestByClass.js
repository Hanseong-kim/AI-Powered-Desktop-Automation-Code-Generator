...
//   [W1] "(unknown)" (main)
//   [W2] "제목 없음 - 메모장" (opened during recording)

class NotepadPageByClass {

    // ════════════════════════════════════════════════════════════
    // [W2] 제목 없음 - 메모장 (new window)
    // ════════════════════════════════════════════════════════════
    async click2() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"","className":"RichEditD2DPT","name":"텍스트 편집기"});
    }

    async type3(value) {
        const ok = await _typeScopedOrCom('제목 없음 - 메모장', '//Document[@ClassName="RichEditD2DPT" and @Name="텍스트 편집기"]', value);
        if (!ok) {
            console.warn('[type3] scoped sendKeys failed — falling back to OS-level typing');
            osActivate('제목 없음 - 메모장', _hwndCache['제목 없음 - 메모장']);
            osType(value);
        }
    }
    ...
    async click5() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"","className":"ToggleButton","name":"굵게(Ctrl+B)"});
    }
    ...
    async click8() {
        osExpandCollapse(_hwndCache[_mainTitleFrag], {"automationId":"File","className":"Microsoft.UI.Xaml.Controls.MenuBarItem","name":"파일"}, "새 탭");
    }

    async click9() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"CloseButton","className":"Button","name":"탭 닫기"});
    }

    async click10() {
        osExpandCollapse(_hwndCache[_mainTitleFrag], {"automationId":"File","className":"Microsoft.UI.Xaml.Controls.MenuBarItem","name":"파일"}, "새 창");
    }
...
async function run() {
    ...
    _mainTitleFrag = "제목 없음 - 메모장";
    _dialogRects = {"제목 없음 - 메모장":{"left":200,"top":125,"width":1034,"height":895}};
    await ensureAppium();
    _rootSid = await _createSession('Root');
    console.log(`[session] Root session ${_rootSid} ready`);
        await launchApp("C:\\Windows\\System32\\notepad.exe", [], "제목 없음 - 메모장", {"left":465,"top":73,"width":1034,"height":895});

        const page = new NotepadPageByClass();

    // ════════════════════════════════════════════════════════════
    // [W1] (unknown) (main window)
    // ════════════════════════════════════════════════════════════
    ...
    // [W2] 제목 없음 - 메모장 (new window)
    // ════════════════════════════════════════════════════════════
            await _step('2:click 텍스트 편집기 (cross-window)', () => page.click2());
            await _step('3:type hi nmy is handung\n', () => page.type3('hi nmy is handung\n'));
            await _step('4:type \n', () => page.type4('\n'));
            await _step('5:click 굵게(Ctrl+B) (cross-window)', () => page.click5());
            await _step('6:type lets see what i do\n', () => page.type6('lets see what i do\n'));
            await _step('7:type \n', () => page.type7('\n'));
            await _step('8:expandCollapse 파일 -> 새 탭', () => page.click8());
            await _step('9:click 탭 닫기 (cross-window)', () => page.click9());
            await _step('10:expandCollapse 파일 -> 새 창', () => page.click10());
            await _step('11:click 탭 닫기 (cross-window)', () => page.click11());
    } finally {
