...
class NotepadPageByClass {
    async click1() {
        const el = await browser.$('//Button[@ClassName="Button" and @Name="새 탭 추가"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }

    async click2() {
        const el = await browser.$('//Document[@ClassName="RichEditD2DPT" and @Name="텍스트 편집기"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }
    ...
    async type3(value) {
        try {
            ...
            console.warn('[type3] element sendKeys failed — falling back to OS-level typing:', String(e.message || e).substring(0, 100));
            osActivate('');
            osType(value);
        }
    }
    ...
    async type4(value) {
        try {
            ...
            // amount of retrying helps, so fall back to real OS-level key
            // injection instead of failing the step.
            console.warn('[type4] element sendKeys failed — falling back to OS-level typing:', String(e.message || e).substring(0, 100));
            osActivate('');
            osType(value);
        }
    }
    ...
    async type5(value) {
        try {
            ...
            // amount of retrying helps, so fall back to real OS-level key
            // injection instead of failing the step.
            console.warn('[type5] element sendKeys failed — falling back to OS-level typing:', String(e.message || e).substring(0, 100));
            osActivate('');
            osType(value);
        }
    }
    ...
    async type6(value) {
        try {
            ...
            // amount of retrying helps, so fall back to real OS-level key
            // injection instead of failing the step.
            console.warn('[type6] element sendKeys failed — falling back to OS-level typing:', String(e.message || e).substring(0, 100));
            osActivate('');
            osType(value);
        }
    }
    ...
    async type7(value) {
        try {
            ...
            // amount of retrying helps, so fall back to real OS-level key
            // injection instead of failing the step.
            console.warn('[type7] element sendKeys failed — falling back to OS-level typing:', String(e.message || e).substring(0, 100));
            osActivate('');
            osType(value);
        }
    }
    ...
    async type8(value) {
        try {
            ...
            // amount of retrying helps, so fall back to real OS-level key
            // injection instead of failing the step.
            console.warn('[type8] element sendKeys failed — falling back to OS-level typing:', String(e.message || e).substring(0, 100));
            osActivate('');
            osType(value);
        }
    }
    ...
    async click9() {
        const el = await browser.$('//MenuItem[@ClassName="Microsoft.UI.Xaml.Controls.MenuBarItem" and @Name="파일"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }

    async click10() {
        const el = await browser.$('//MenuItem[@ClassName="Microsoft.UI.Xaml.Controls.MenuBarItem" and @Name="편집"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }

    async click11() {
        const el = await browser.$('//MenuItem[@ClassName="Microsoft.UI.Xaml.Controls.MenuBarItem" and @Name="보기"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }

    async click12() {
        const el = await browser.$('//Button[@ClassName="ToggleButton" and @Name="굵게(Ctrl+B)"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }
}
...
describe('NotepadTestByClass', () => {
    beforeAll(async () => {
        _warmupPowerShell();
        await initAppHwnd();
        normalizeWindowSimple({"left":160,"top":100,"width":827,"height":716});
    });

    it('should replay recorded flow', async () => {
        const page = new NotepadPageByClass();

            osActivate("*🗺️ 2박 3일 편도 로드트립 타임라인 - 메모장");
            await _step('1:click 새 탭 추가', () => page.click1());
            await _step('2:click 텍스트 편집기', () => page.click2());
            await _step('3:type asdf\n', () => page.type3('asdf\n'));
            await _step('4:type \n', () => page.type4('\n'));
            await _step('5:type asdfasdf\n', () => page.type5('asdfasdf\n'));
            await _step('6:type \n', () => page.type6('\n'));
            await _step('7:type 123412344qwerqwer\n', () => page.type7('123412344qwerqwer\n'));
            await _step('8:type \n', () => page.type8('\n'));
            await _step('9:click 파일', () => page.click9());
            await _step('10:click 편집', () => page.click10());
            await _step('11:click 보기', () => page.click11());
            await _step('12:click 굵게(Ctrl+B)', () => page.click12());
