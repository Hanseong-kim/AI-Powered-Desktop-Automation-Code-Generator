...
class NotepadPageById {
    async click1() {
        const el = await browser.$('~AddButton');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }

    async click2() {
        const el = await browser.$('//*[@Name="텍스트 편집기"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }
    ...
    async click9() {
        const el = await browser.$('~File');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }

    async click10() {
        const el = await browser.$('~Edit');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }

    async click11() {
        const el = await browser.$('~View');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }

    async click12() {
        const el = await browser.$('//*[@Name="굵게(Ctrl+B)"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }
}
