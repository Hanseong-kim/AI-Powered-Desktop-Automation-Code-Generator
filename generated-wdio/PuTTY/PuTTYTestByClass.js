...
class PuTTYPageByClass {
    async click1() {
        const el = await browser.$('//TreeItem[@Name="Logging"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }

    async click2() {
        const el = await browser.$('//TreeItem[@Name="Keyboard"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }
    ...
    async click6() {
        const el = await browser.$('//TreeItem[@Name="Selection"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }
    ...
    async click8() {
        const el = await browser.$('//CheckBox[@ClassName="Button" and @Name="System menu appears on ALT-Space"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
        await el.click();
    }
    ...
    async click10() {
        const el = await browser.$('//Button[@ClassName="Button" and @Name="Open"]');
        await el.waitForExist({ timeout: 8000 });
        await el.click();
    }
}
