...
class TeamViewerPageByClass {
    ...
    async click15() {
        osScopedInvoke(_appHwnd, {"automationId":"","className":"","name":"TeamViewer에 로그인"});
    }

    async click16() {
        osScopedInvoke(_appHwnd, {"automationId":"email-field","className":"","name":"이메일"});
    }
    ...
    async click18() {
        osScopedInvoke(_appHwnd, {"automationId":"","className":"","name":"계속"});
    }
}
...
async function run() {
    ...
    await ensureAppium();
    _appSid = await _createSession("C:\\\\Program Files\\\\TeamViewer\\\\TeamViewer.exe");
    console.log(`[session] app session ${_appSid} ready`);
    await initAppHwnd();
    normalizeWindowSimple({"left":427,"top":14,"width":1300,"height":978});

        const page = new TeamViewerPageByClass();
            osActivate("TeamViewer");
