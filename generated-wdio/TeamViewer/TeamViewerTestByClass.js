...
const _H = {
    ...
    'osA11yWarmup.py': "import sys, argparse, ctypes, time\nfrom ctypes import wintypes\n\nif sys.stdout.encoding and sys.stdout.encoding.lower() not in (\"utf-8\", \"utf8\"):\n    sys.stdout.reconfigure(encoding=\"utf-8\", errors=\"replace\")\n\nimport comtypes\nimport comtypes.client\n\nTreeScope_Subtree = 7\nuser32 = ctypes.windll.user32\n\n# Chromium/WebView2 호스트가 만드는 실제 Win32 자식 창 클래스들(접두사 매칭).\n# 이 중 하나라도 자식으로 있을 때만 웜업을 돈다 — 순수 Win32 앱은 즉시 빠져나간다.\n# TeamViewer 실측 자식 목록: TV_WebView2Control, Chrome_WidgetWin_0/_1.\n# Electron/CEF은 Chrome_WidgetWin_1 + Chrome_RenderWidgetHostHWND로 나타난다.\nCHROMIUM_CLASSES = (\n    \"Chrome_WidgetWin_\", \"Chrome_RenderWidgetHostHWND\", \"TV_WebView2Control\",\n    \"Intermediate D3D Window\", \"WebView2\", \"CefBrowserWindow\",\n)\n\n\ndef get_uia():\n    comtypes.CoInitialize()\n    mod = comtypes.client.GetModule(\"UIAutomationCore.dll\")\n    uia = comtypes.client.CreateObject(\n        \"{ff48dba4-60ef-4201-aa87-54103eef594e}\", interface=mod.IUIAutomation)\n    return uia, mod\n\n\ndef subtree_size(uia, win):\n    try:\n        return win.FindAll(TreeScope_Subtree, uia.CreateTrueCondition()).Length\n    except Exception:\n        return -1\n\n\n# 호스트 판정은 반드시 Win32 EnumChildWindows로 한다 — UIA 서브트리로 찾으면\n# 안 된다(2026-07-31 실측): 웜업이 필요한 바로 그 상태에서는 UIA 트리가 통째로\n# 비어 있어(측정치 subtree=2, TV_WebView2Control조차 안 보임) \"Chromium 아님\"\n# 으로 오판하고 그냥 빠져나간다 — 즉 자기가 고쳐야 할 상황에서만 작동을\n# 멈추는 판정이 된다. 자식 창 클래스는 UIA 접근성 활성화 여부와 무관한\n# 순수 Win32 사실이라 항상 읽을 수 있다.\ndef chromium_children(hwnd):\n    \"\"\"(childHwnd, className) of every Chromium/WebView2 renderer child window.\"\"\"\n    found = []\n\n    @ctypes.WINFUNCTYPE(ctypes.c_bool, wintypes.HWND, wintypes.LPARAM)\n    def cb(h, _):\n        buf = ctypes.create_unicode_buffer(256)\n        user32.GetClassNameW(h, buf, 256)\n        cls = buf.value\n        if any(cls.startswith(m) for m in CHROMIUM_CLASSES):\n            found.append((h, cls))\n        return True\n\n    try:\n        user32.EnumChildWindows(hwnd, cb, 0)\n    except Exception:\n        return []\n    return found\n\n\ndef looks_chromium(hwnd):\n    return bool(chromium_children(hwnd))\n\n\ndef main():\n    ap = argparse.ArgumentParser()\n    ap.add_argument(\"--hwnd\", type=int, required=True)\n    args = ap.parse_args()\n\n    if not user32.IsWindow(args.hwnd):\n        print(\"osA11yWarmup: not a window: %d\" % args.hwnd, file=sys.stderr)\n        return 1\n\n    uia, mod = get_uia()\n    try:\n        win = uia.ElementFromHandle(args.hwnd)\n    except Exception as e:\n        print(\"osA11yWarmup: ElementFromHandle failed: %s\" % e, file=sys.stderr)\n        return 1\n\n    if not looks_chromium(args.hwnd):\n        print(\"[osA11yWarmup] no Chromium/WebView2 host in this window - nothing to warm up\")\n        return 0\n\n    before = subtree_size(uia, win)\n\n    # 깨우는 방법은 hwnd 지정 ElementFromHandle이다 — ElementFromPoint(좌표\n    # 히트테스트)를 쓰면 안 된다(2026-07-31 실측): 그건 화면 좌표 기준이라 대상\n    # 창이 가려져 있거나 포그라운드가 아니면 맨 위에 있는 **다른 앱**을 찔러\n    # 정작 렌더러는 건드리지도 못하고(측정: subtree 2 -> 2 그대로), 부작용으로\n    # 남의 창을 건드린다. ElementFromHandle은 그 창 하나에만 WM_GETOBJECT를\n    # 보내므로 z-order/커서와 무관하고 좌표도 안 쓴다(§3에도 더 부합).\n    #\n    # 실측(TeamViewer, 창이 가려진 상태):\n    #   ElementFromHandle(Chrome_WidgetWin_0) -> 서브트리 55개 (웹 콘텐츠 전부)\n    #   ElementFromHandle(Chrome_RenderWidgetHostHWND) -> 39개\n    # 즉 콘텐츠는 Chromium 자식 hwnd를 루트로 잡으면 확실히 보인다.\n    woken = 0\n    for h, cls in chromium_children(args.hwnd):\n        try:\n            child_el = uia.ElementFromHandle(h)\n            n = subtree_size(uia, child_el)\n            if n > 1:\n                woken += 1\n            print(\"[osA11yWarmup]   %s (hwnd=%d) exposes %d elements\" % (cls, h, n))\n        except Exception as e:\n            print(\"[osA11yWarmup]   %s (hwnd=%d) ElementFromHandle failed: %s\"\n                  % (cls, h, e), file=sys.stderr)\n\n    after = subtree_size(uia, win)\n    for _ in range(10):\n        if after > before:\n            break\n        time.sleep(0.3)\n        after = subtree_size(uia, win)\n\n    print(\"[osA11yWarmup] Chromium host detected - woke %d renderer window(s), \"\n          \"main-window UIA subtree %d -> %d elements\" % (woken, before, after))\n    if after <= before:\n        # 메인 창 서브트리가 안 늘어도 렌더러 자체는 깨어 있을 수 있다 —\n        # 실측상 TeamViewer는 창이 비활성일 때 메인 창 트리에 WebView2 자식을\n        # 아예 안 붙인다(subtree=2 고정). 이 경우 메인 창 기준 하향 검색은\n        # 계속 실패하므로, 재생 전에 창을 활성화하는 쪽이 선행되어야 한다.\n        print(\"[osA11yWarmup] WARN the main window's own subtree did not grow - \"\n              \"content is reachable from the renderer hwnd but not from the app \"\n              \"window; downward searches rooted at the app window will still miss it\",\n              file=sys.stderr)\n    return 0\n\n\nif __name__ == \"__main__\":\n    sys.exit(main())\n",
};
let _helperDir = null;
...
// Chromium/WebView2 접근성 웜업 — 창을 잡은 직후 1회. 순수 Win32 앱에서는
// 헬퍼가 즉시 빠져나오므로 사실상 무비용이다. 실패해도 재생을 막지 않는다
// (웜업이 필요 없는 앱이 대다수이고, 필요한 앱에서는 뒤이은 element 조회가
// 어차피 진짜 실패로 잡힌다).
const _a11yWarmedHwnds = new Set();
function osA11yWarmup(hwnd) {
    if (!hwnd || _a11yWarmedHwnds.has(hwnd)) return;
    _a11yWarmedHwnds.add(hwnd);
    try {
        const out = execSync(
            `python "${_helperFile('osA11yWarmup.py')}" --hwnd ${hwnd}`,
            { stdio: 'pipe', timeout: 30000 }
        ).toString().trim();
        if (out) console.log(out);
    } catch (e) {
        console.warn('[osA11yWarmup] failed:',
            String((e.stderr && e.stderr.toString()) || e.message || e).slice(-300));
    }
}
...
async function _findScoped(title, selector, timeoutMs = 8000) {
    ...
    for (;;) {
        ...
        // 이 창이 Chromium/WebView2 호스트면 하향 검색 전에 접근성 트리를
        // 깨운다 (창당 1회, Set이 중복 호출을 막는다). 팝업/알림 다이얼로그와
        // 화면 전환 뒤 새 창이 여기로 들어오므로, 메인 창만 웜업하던 경우
        // 놓치던 케이스를 함께 덮는다.
        osA11yWarmup(s.hwnd);
...
async function launchApp(exePath, args, titleFrag, rect) {
    ...
    while (Date.now() < deadline) {
        ...
        if (liveRect && liveRect.width > 0 && liveRect.height > 0) {
            ...
            // Chromium/WebView2 호스트라면 접근성 트리를 깨운다 — 그 전에는
            // 하향 트리 검색(FindFirst/WinAppDriver /element)에 웹 콘텐츠가
            // 하나도 안 잡힌다. 창 크기가 확정된 뒤여야 히트테스트가 유효하다.
            osA11yWarmup(_hwndCache[titleFrag]);
            return;
        }
        await new Promise(r => setTimeout(r, 1000));
    }
...
//   [W1] "TeamViewer" (main)
//   [W2] "TeamViewer" (opened during recording)
//   [W3] "빠른 연결 허용" (opened during recording)
//   [W4] "오류" (opened during recording)
//   [W5] "TeamViewer" (opened during recording)

class TeamViewerPageByClass {

    // ════════════════════════════════════════════════════════════
    // [W1] TeamViewer (main window)
    // ════════════════════════════════════════════════════════════
    async click1() {
        await _clickScoped('TeamViewer', '~TextField113');
    }

    async type2(value) {
        const ok = await _typeScopedOrCom('TeamViewer', '~TextField113', value);
        if (!ok) {
            console.warn('[type2] scoped sendKeys failed — falling back to OS-level typing');
            osActivate('TeamViewer', _hwndCache['TeamViewer']);
            osType(value);
        }
    }

    async click3() {
        await _clickScoped('TeamViewer', '//Button[@Name="세션 참가"]');
    }


    // ════════════════════════════════════════════════════════════
    // [W2] TeamViewer (new window)
    // ════════════════════════════════════════════════════════════
    async click4() {
        await _clickScoped('TeamViewer', '//Window[@ClassName="#32770" and @Name="TeamViewer"]');
    }

    async click5() {
        await _clickScoped('TeamViewer', '//CheckBox[@Name="이 장치에 Easy Access 권한 부여"]');
    }
    ...
    // ════════════════════════════════════════════════════════════
    async click6() {
        await _clickScoped('빠른 연결 허용', '//Edit[@ClassName="Edit" and @Name="이메일"]');
    }

    async type7(value) {
        const ok = await _typeScopedOrCom('빠른 연결 허용', '//Edit[@ClassName="Edit" and @Name="이메일"]', value);
        if (!ok) {
            console.warn('[type7] scoped sendKeys failed — falling back to OS-level typing');
            osActivate('빠른 연결 허용', _hwndCache['빠른 연결 허용']);
            osType(value);
        }
    }

    async click8() {
        await _clickScoped('빠른 연결 허용', '//Edit[@ClassName="Edit" and @Name="비밀번호"]');
    }

    async type9(value) {
        const ok = await _typeScopedOrCom('빠른 연결 허용', '//Edit[@ClassName="Edit" and @Name="비밀번호"]', value);
        if (!ok) {
    ...
    async click10() {
        await _clickScoped('빠른 연결 허용', '//Button[@ClassName="Button" and @Name="할당"]');
    }
    ...
    // ════════════════════════════════════════════════════════════
    async click11() {
        await _clickScoped('오류', '//Window[@ClassName="#32770" and @Name="오류"]');
    }


    // ════════════════════════════════════════════════════════════
    // [W5] TeamViewer (new window)
    // ════════════════════════════════════════════════════════════
    async click12() {
        await _clickScoped('TeamViewer', '//Button[@Name="TeamViewer에 로그인"]');
    }

    async click13() {
        await _clickScoped('TeamViewer', '//Pane[@ClassName="View"]');
    }
}
...
async function run() {
    ...
    _mainTitleFrag = "TeamViewer";
    _dialogRects = {"TeamViewer":{"left":337,"top":48,"width":1300,"height":977},"빠른 연결 허용":{"left":758,"top":382,"width":459,"height":309},"오류":{"left":632,"top":346,"width":711,"height":381}};
    await ensureAppium();
    _rootSid = await _createSession('Root');
    console.log(`[session] Root session ${_rootSid} ready`);
        await launchApp("C:\\Program Files\\TeamViewer\\TeamViewer.exe", [], "TeamViewer", {"left":337,"top":48,"width":1300,"height":977});

        const page = new TeamViewerPageByClass();

    // ════════════════════════════════════════════════════════════
    // [W1] TeamViewer (main window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: TeamViewer', async () => { await _switchWindow('TeamViewer'); });
            await _step('1:click 세션 코드', () => page.click1());
            await _step('2:type 123456789', () => page.type2('123456789'));
            await _step('3:click 세션 참가', () => page.click3());

    // ════════════════════════════════════════════════════════════
    // [W2] TeamViewer (new window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: TeamViewer', async () => { await _switchWindow('TeamViewer'); });
            await _step('4:click TeamViewer', () => page.click4());
            await _step('5:click 이 장치에 Easy Access 권한 부여', () => page.click5());

    // ════════════════════════════════════════════════════════════
    // [W3] 빠른 연결 허용 (new window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: 빠른 연결 허용', async () => { await _switchWindow('빠른 연결 허용'); });
            await _step('6:click 이메일', () => page.click6());
            await _step('7:type asdf', () => page.type7('asdf'));
            await _step('8:click 비밀번호', () => page.click8());
            await _step('9:type asdf', () => page.type9('asdf'));
            await _step('10:click 할당', () => page.click10());

    // ════════════════════════════════════════════════════════════
    // [W4] 오류 (new window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: 오류', async () => { await _switchWindow('오류'); });
            await _step('11:click 오류', () => page.click11());

    // ════════════════════════════════════════════════════════════
    // [W5] TeamViewer (new window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: TeamViewer', async () => { await _switchWindow('TeamViewer'); });
            await _step('12:click TeamViewer에 로그인', () => page.click12());
            await _step('13:click ', () => page.click13());
    } finally {
