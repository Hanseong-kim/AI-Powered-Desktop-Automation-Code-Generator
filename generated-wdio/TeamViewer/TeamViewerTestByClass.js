...
class TeamViewerPageByClass {
    ...
    async click2() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"","className":"","name":"ID를 복사하세요"}, {"automationId":"","className":"","name":"비밀번호를 새로고침하세요"}, 315, 489, "TeamViewer");
    }

    async click3() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"","className":"","name":"비밀번호를 복사하세요"}, null, 400, null, "TeamViewer");
    }

    async click4() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"","className":"","name":"비밀번호를 새로고침하세요"}, null, 403, null, "TeamViewer");
    }

    async click5() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"","className":"","name":"세션 참가"}, null, 520, null, "TeamViewer");
    }
...
async function run() {
    ...
    _dialogRects = {"TeamViewer":{"left":337,"top":48,"width":1300,"height":977},"빠른 연결 허용":{"left":2459,"top":285,"width":394,"height":250}};
    await ensureAppium();
    ...
    // [W1] TeamViewer (main window)
    // ════════════════════════════════════════════════════════════
            await _step('1:click 비밀번호를 복사하세요', () => page.click1());
            await _step('2:click ID를 복사하세요 (cross-window)', () => page.click2());
            await _step('3:click 비밀번호를 복사하세요 (cross-window)', () => page.click3());
            await _step('4:click 비밀번호를 새로고침하세요 (cross-window)', () => page.click4());
            await _step('5:click 세션 참가 (cross-window)', () => page.click5());

    // ════════════════════════════════════════════════════════════
    // [W2] TeamViewer (new window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: TeamViewer', async () => { await _switchWindow('TeamViewer'); });
            await _step('6:click TeamViewer', () => page.click6());
            ...
            // [STEP 9] click: no selector/anchor captured — coordinate replay is forbidden (2026-07-10)
            _failures.push('9:click:no-selector');

    // ════════════════════════════════════════════════════════════
    // [W3] 빠른 연결 허용 (new window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: 빠른 연결 허용', async () => { await _switchWindow('빠른 연결 허용'); });
            await _step('10:click 이메일', () => page.click10());
            await _step('11:type asdf', () => page.type11('asdf'));
            await _step('12:click 비밀번호', () => page.click12());
