...
class TeamViewerPageByClass {
    async click2() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"email-field","className":"","name":"이메일"});
    }
    ...
    async click4() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"","className":"","name":"계속","controlTypeId":50000});
    }
    ...
    async type8(value) {
        ...
        if (!hs.length) {
            console.warn('[type8] window "빠른 연결 허용" not found by EnumWindows — activation/typing may miss');
        }
...
async function run() {
    ...
    _dialogRects = {"TeamViewer":{"left":427,"top":14,"width":1300,"height":976},"빠른 연결 허용":{"left":848,"top":348,"width":459,"height":309}};
    await ensureAppium();
    _rootSid = await _createSession('Root');
    console.log(`[session] Root session ${_rootSid} ready`);
        await launchApp("C:\\Program Files\\TeamViewer\\TeamViewer.exe", [], "TeamViewer", {"left":427,"top":14,"width":1300,"height":976});
    ...
    // ════════════════════════════════════════════════════════════
            ...
            console.log('[STEP] switch to window: TeamViewer (skipped — no selector for this step)');
            _failures.push('1:click:no-selector');
            await _step('2:click 이메일', () => page.click2());
            await _step('3:type asdfasdf', () => page.type3('asdfasdf'));
            await _step('4:click 계속', () => page.click4());
    ...
    // ════════════════════════════════════════════════════════════
            ...
            await _step('7:click 이메일', () => page.click7());
            await _step('8:type asdf', () => page.type8('asdf'));
            await _step('9:click 비밀번호', () => page.click9());
            await _step('10:type asdf', () => page.type10('asdf'));
            await _step('11:click 빠른 연결 허용', () => page.click11());
            await _step('12:click 취소', () => page.click12());
    } finally {
