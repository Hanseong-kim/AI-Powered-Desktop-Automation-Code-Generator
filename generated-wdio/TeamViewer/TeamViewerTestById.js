...
class TeamViewerPageById {
    ...
    async type10(value) {
        const ok = await _typeScoped(_appSid, null, '~1125', value);
        if (!ok) {
    ...
    async type13(value) {
        const ok = await _typeScoped(_appSid, null, '~1126', value);
        if (!ok) {
            // WinAppDriver's element/value endpoint rejects some native edit
            // controls outright (confirmed 2026-07-08: Win11 Notepad's
            // RichEditD2DPT Document control returns "unknown error" in
...
async function run() {
    ...
    await initAppHwnd();
    normalizeWindowSimple({"left":2139,"top":84,"width":1040,"height":782});
