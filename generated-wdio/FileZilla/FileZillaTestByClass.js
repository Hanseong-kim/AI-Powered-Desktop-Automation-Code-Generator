...
class FileZillaPageByClass {
    ...
    async type4(value) {
        ...
        if (!hs.length) {
            console.warn('[type4] window "사이트 관리자" not found by EnumWindows — activation/typing may miss');
        }
    ...
    async click5() {
        await _clickScoped('사이트 관리자', '//Tree[@ClassName="SysTreeView32" and @Name="항목 선택(S):"]');
    }
    ...
    async type7(value) {
        const hs = _listWindowHwnds('사이트 관리자');
        if (!hs.length) {
            console.warn('[type7] window "사이트 관리자" not found by EnumWindows — activation/typing may miss');
        }
        osActivate('사이트 관리자', hs[0]);
        osType(value);
    }
    ...
    async click13() {
        await _clickScoped('사이트 관리자', '//Edit[@ClassName="Edit" and @Name="사용자(U):"]');
    }

    async type14(value) {
        const hs = _listWindowHwnds('사이트 관리자');
        if (!hs.length) {
            console.warn('[type14] window "사이트 관리자" not found by EnumWindows — activation/typing may miss');
        }
        osActivate('사이트 관리자', hs[0]);
        osType(value);
    }
    ...
    async click15() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"","className":"","name":"고급"}, null, 67, null, "사이트 관리자");
    }

    async click16() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"","className":"","name":"전송 설정"}, null, 64, null, "사이트 관리자");
    }

    async click17() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"","className":"","name":"문자셋"}, null, 59, null, "사이트 관리자");
    }

    async click18() {
        osScopedInvoke(_hwndCache[_mainTitleFrag], {"automationId":"","className":"","name":"일반"}, null, 60, null, "사이트 관리자");
    }
...
async function run() {
    ...
    _dialogRects = {"FileZilla":{"left":1889,"top":116,"width":1200,"height":947},"사이트 관리자":{"left":676,"top":205,"width":1182,"height":639},"새 북마크":{"left":1047,"top":332,"width":440,"height":385}};
    await ensureAppium();
    _rootSid = await _createSession('Root');
    console.log(`[session] Root session ${_rootSid} ready`);
        await launchApp("C:\\Program Files\\FileZilla FTP Client\\filezilla.exe", [], "FileZilla", {"left":1889,"top":116,"width":1200,"height":947});
    ...
    // ════════════════════════════════════════════════════════════
            ...
            await _step('2:select item #0 사이트 관리자(S)...\tCtrl+S', () => page.click2());

    // ════════════════════════════════════════════════════════════
    // [W2] 사이트 관리자 (new window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: 사이트 관리자', async () => { await _switchWindow('사이트 관리자'); });
            await _step('3:click 새 폴더(F)', () => page.click3());
            await _step('4:type asdf', () => page.type4('asdf'));
            await _step('5:click 항목 선택(S):', () => page.click5());
            await _step('6:click 새 사이트(N)', () => page.click6());
            await _step('7:type qwer', () => page.type7('qwer'));
            await _step('8:click 항목 선택(S):', () => page.click8());
            ...
            await _step('13:click 사용자(U):', () => page.click13());
            await _step('14:type asdf', () => page.type14('asdf'));
            await _step('15:click 고급 (cross-window)', () => page.click15());
            await _step('16:click 전송 설정 (cross-window)', () => page.click16());
            await _step('17:click 문자셋 (cross-window)', () => page.click17());
            await _step('18:click 일반 (cross-window)', () => page.click18());
            await _step('19:click 취소', () => page.click19());
            await _step('20:select item #0 북마크 추가(A)...\tCtrl+B', () => page.click20());

    // ════════════════════════════════════════════════════════════
    // [W3] 새 북마크 (new window)
    // ════════════════════════════════════════════════════════════
            await _step('21:click 탐색 동기화 사용(S)', () => page.click21());
            await _step('22:click 디렉터리 비교(I)', () => page.click22());
            await _step('23:click 탐색 동기화 사용(S)', () => page.click23());
            await _step('24:click 디렉터리 비교(I)', () => page.click24());
