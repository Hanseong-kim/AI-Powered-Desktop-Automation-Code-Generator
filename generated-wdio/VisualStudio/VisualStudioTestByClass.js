...
class VisualStudioPageByClass {
    async click1() {
        osExpandCollapse(_appHwnd, {"automationId":"ComboBox_3","className":"ComboBox","name":"프로젝트 형식 필터"}, null, null, null);
    }

    async click2() {
        osExpandCollapse(_appHwnd, {"automationId":"ComboBox_1","className":"ComboBox","name":"언어 필터"}, null, null, null);
    }

    async click3() {
        osExpandCollapse(_appHwnd, {"automationId":"ComboBox_2","className":"ComboBox","name":"플랫폼 필터"}, null, null, null);
    }

    async click4() {
        await _clickBySid(_appSid, null, '//Text[@ClassName="TextBlock" and @Name="뒤로(_B)"]');
    }
...
async function run() {
    ...
    await ensureAppium();
    _appSid = await _createSession("C:\\\\Program Files\\\\Microsoft Visual Studio\\\\18\\\\Community\\\\Common7\\\\IDE\\\\devenv.exe");
    console.log(`[session] app session ${_appSid} ready`);
    await initAppHwnd();
    normalizeWindowSimple({"left":398,"top":174,"width":1125,"height":673});

        const page = new VisualStudioPageByClass();
            osActivate("Microsoft Visual Studio");
            await _step('1:expandCollapse 프로젝트 형식 필터', () => page.click1());
            await _step('2:expandCollapse 언어 필터', () => page.click2());
            await _step('3:expandCollapse 플랫폼 필터', () => page.click3());
            await _step('4:click 뒤로(_B)', () => page.click4());
            // [STEP 5] click: no selector/anchor captured — coordinate replay is forbidden (2026-07-10)
            _failures.push('5:click:no-selector');
            await _step('6:click 뒤로(_B)', () => page.click6());
    } finally {
