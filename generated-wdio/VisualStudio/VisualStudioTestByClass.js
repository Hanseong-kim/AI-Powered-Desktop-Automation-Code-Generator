...
//   [W1] "Microsoft Visual Studio (관리자)" (main)
//   [W2] "hansung - Microsoft Visual Studio (관리자)" (opened during recording)

class VisualStudioPageByClass {

    // ════════════════════════════════════════════════════════════
    // [W1] Microsoft Visual Studio (관리자) (main window)
    // ════════════════════════════════════════════════════════════
    async click1() {
        await _clickScoped('Microsoft Visual Studio (관리자)', '//Text[@ClassName="TextBlock" and @Name="C:\\"]', false, 50020);
    }


    // ════════════════════════════════════════════════════════════
    // [W2] hansung - Microsoft Visual Studio (관리자) (new window)
    // ════════════════════════════════════════════════════════════
    async click2() {
        let _expHs2 = _listWindowHwnds('hansung - Microsoft Visual Studio (관리자)');
        if (!_expHs2.length) {
            const _expTail2 = 'hansung - Microsoft Visual Studio (관리자)'.split(' - ').pop();
            if (_expTail2 !== 'hansung - Microsoft Visual Studio (관리자)') _expHs2 = _listWindowHwnds(_expTail2);
        }
        const _expHwnd2 = _expHs2.find(h => h === osForegroundHwnd()) || _expHs2[0] || _hwndCache[_mainTitleFrag];
        osExpandCollapse(_expHwnd2, {"automationId":"","className":"MenuItem","name":"파일"}, null, null, null);
    }

    async click3() {
        await _clickScoped('hansung - Microsoft Visual Studio (관리자)', '//MenuItem[@ClassName="MenuItem" and @Name="파일"]', false, 50011);
    }

    async click4() {
        let _expHs4 = _listWindowHwnds('hansung - Microsoft Visual Studio (관리자)');
        if (!_expHs4.length) {
            const _expTail4 = 'hansung - Microsoft Visual Studio (관리자)'.split(' - ').pop();
            if (_expTail4 !== 'hansung - Microsoft Visual Studio (관리자)') _expHs4 = _listWindowHwnds(_expTail4);
        }
        const _expHwnd4 = _expHs4.find(h => h === osForegroundHwnd()) || _expHs4[0] || _hwndCache[_mainTitleFrag];
        osExpandCollapse(_expHwnd4, {"automationId":"","className":"MenuItem","name":"보기"}, null, null, null);
    }

    async click5() {
        await _clickScoped('hansung - Microsoft Visual Studio (관리자)', '//MenuItem[@ClassName="MenuItem" and @Name="보기"]', false, 50011);
    }

    async click6() {
        let _expHs6 = _listWindowHwnds('hansung - Microsoft Visual Studio (관리자)');
        if (!_expHs6.length) {
            const _expTail6 = 'hansung - Microsoft Visual Studio (관리자)'.split(' - ').pop();
            if (_expTail6 !== 'hansung - Microsoft Visual Studio (관리자)') _expHs6 = _listWindowHwnds(_expTail6);
        }
        const _expHwnd6 = _expHs6.find(h => h === osForegroundHwnd()) || _expHs6[0] || _hwndCache[_mainTitleFrag];
        osExpandCollapse(_expHwnd6, {"automationId":"","className":"MenuItem","name":"파일"}, null, null, null);
    }

    async click7() {
        await _clickScoped('hansung - Microsoft Visual Studio (관리자)', '//Button[@ClassName="Button" and @Name="최소화"]', false, 50000);
    }
}
...
async function run() {
    ...
    _mainTitleFrag = "Microsoft Visual Studio (관리자)";
    _dialogRects = {"hansung - Microsoft Visual Studio (관리자)":{"left":-9,"top":-9,"width":1938,"height":1038}};
    await ensureAppium();
    _rootSid = await _createSession('Root');
    console.log(`[session] Root session ${_rootSid} ready`);
        await launchApp("C:\\Program Files\\Microsoft Visual Studio\\18\\Community\\Common7\\IDE\\devenv.exe", [], "Microsoft Visual Studio (관리자)", {"left":-9,"top":-9,"width":1938,"height":1038});

        const page = new VisualStudioPageByClass();

    // ════════════════════════════════════════════════════════════
    // [W1] Microsoft Visual Studio (관리자) (main window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: Microsoft Visual Studio (관리자)', async () => { await _switchWindow('Microsoft Visual Studio (관리자)'); osActivate('Microsoft Visual Studio (관리자)', _hwndCache['Microsoft Visual Studio (관리자)']); });
            await _step('1:click C:\\', () => page.click1());

    // ════════════════════════════════════════════════════════════
    // [W2] hansung - Microsoft Visual Studio (관리자) (new window)
    // ════════════════════════════════════════════════════════════
            await _step('2:expandCollapse 파일', () => page.click2());
            await _step('3:click 파일', () => page.click3());
            await _step('4:expandCollapse 보기', () => page.click4());
            await _step('5:click 보기', () => page.click5());
            await _step('6:expandCollapse 파일', () => page.click6());
            await _step('7:click 최소화', () => page.click7());
    } finally {
