...
class HeidiSQLPageById {
    async click1() {
        await _clickBySid(_appSid, null, '//*[@ClassName="TVirtualStringTree"]');
    }
    ...
    async click10() {
        await _clickBySid(_appSid, null, '//Button[@Name="취소"]');
    }
}
...
async function run() {
    ...
    await initAppHwnd();
    normalizeWindowSimple({"left":610,"top":144,"width":1062,"height":800});

        const page = new HeidiSQLPageById();
            osActivate("HeidiSQL 12.20.0.7320 - 세션 관리자: Unnamed");
