...
class VSCodePageById {
    ...
    async click3() {
        ...
        const c = await getCenter(sid, '//*[@Name="즐겨찾기 시작 - 바탕 화면(고정됨)"]') ?? { x: 2406, y: -307 };
        osClick(c.x, c.y);
    }

    async click4() {
        const sid = await getWindowSession('폴더 열기');
        const c = await getCenter(sid, '//*[@Name="폴더 선택(S)"]') ?? { x: 2907, y: -65 };
        osClick(c.x, c.y);
    }
    ...
    async click8() {
        const sid = await getWindowSession('폴더 열기');
        const c = await getCenter(sid, '//*[@Name="live_crop"]') ?? { x: 2406, y: -332 };
        osClick(c.x, c.y);
    }

    async click9() {
        const sid = await getWindowSession('폴더 열기');
        const c = await getCenter(sid, '//*[@Name="폴더 선택(S)"]') ?? { x: 2924, y: -69 };
        osClick(c.x, c.y);
    }
}
