...
class VSCodePageByClass {
    ...
    async click5() {
        let { c, s } = await getCenterWithWait('폴더 열기', '//TreeItem[@Name="로컬 디스크 (C:)"]');
        if (c && s.hwnd) {
    ...
    async click6() {
        let { c, s } = await getCenterWithWait('폴더 열기', '//ListItem[@ClassName="UIItem" and @Name="hansung"]');
        if (c && s.hwnd) {
            const r = _resolveWinRect('폴더 열기');
            if (r) c = { x: r.left + c.x, y: r.top + c.y };
        }
    ...
    async click7() {
        let { c, s } = await getCenterWithWait('폴더 열기', '//ListItem[@ClassName="UIItem" and @Name="project"]');
        if (c && s.hwnd) {
            const r = _resolveWinRect('폴더 열기');
            if (r) c = { x: r.left + c.x, y: r.top + c.y };
        }
    ...
    async click9() {
        let { c, s } = await getCenterWithWait('폴더 열기', '//ListItem[@ClassName="UIItem" and @Name="run"]');
        if (c && s.hwnd) {
            const r = _resolveWinRect('폴더 열기');
            if (r) c = { x: r.left + c.x, y: r.top + c.y };
        }
