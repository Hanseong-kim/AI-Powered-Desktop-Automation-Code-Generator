...
class VSCodePageByClass {
    ...
    async click5() {
        ...
        let c = await getCenter(s.sid, s.rootElId, '//TreeItem[@Name="로컬 디스크 (C:)"]');
        if (!c) {
            delete _sessionIds['폴더 열기'];
            s = await getWindowSession('폴더 열기');
            c = await getCenter(s.sid, s.rootElId, '//TreeItem[@Name="로컬 디스크 (C:)"]');
        }
    ...
    async click7() {
        let s = await getWindowSession('폴더 열기');
        let c = await getCenter(s.sid, s.rootElId, '//Edit[@ClassName="UIProperty" and @Name="이름"]');
        if (!c) {
            delete _sessionIds['폴더 열기'];
            s = await getWindowSession('폴더 열기');
            c = await getCenter(s.sid, s.rootElId, '//Edit[@ClassName="UIProperty" and @Name="이름"]');
        }
    ...
    async click21() {
        let s = await getWindowSession('폴더 열기');
        let c = await getCenter(s.sid, s.rootElId, '//Button[@ClassName="Button" and @Name="폴더 선택(S)"]');
        if (!c) {
            delete _sessionIds['폴더 열기'];
            s = await getWindowSession('폴더 열기');
            c = await getCenter(s.sid, s.rootElId, '//Button[@ClassName="Button" and @Name="폴더 선택(S)"]');
        }
