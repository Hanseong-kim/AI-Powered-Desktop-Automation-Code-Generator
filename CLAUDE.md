...
2. **PoC ③ 다이얼로그 E2E 완주**: PuTTY 등 전통적 Win32 메뉴 앱으로 새 hwnd 스폰→
   scoped 세션 생성→XPath 클릭까지 실측(Paint의 WinUI3 InvokePattern 이슈 회피).
3. FileZilla/PuTTY/7-Zip 등 native 앱으로 녹화→생성→`npx wdio run` end-to-end 검증
...
5. session 모드 doubleClick = 요소 클릭 2회 근사 — 탐색기류 행에서 rename 오발 여부
