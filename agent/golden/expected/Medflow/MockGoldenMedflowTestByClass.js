...
//   [W1] "Medflow Login" (main)
//   [W2] "Medflow Clinical System" (opened during recording)
//   [W3] "Medflow Settings" (opened during recording)
//   [W4] "Medflow Clinical System" (opened during recording)
//   [W5] "Medflow Patient Detail" (opened during recording)

class MockGoldenMedflowPageByClass {

    // ════════════════════════════════════════════════════════════
    // [W1] Medflow Login (main window)
    // ════════════════════════════════════════════════════════════
    async click1() {
        await _clickScoped('Medflow Login', '~userInput', false, 50004);
    }

    async type2(value) {
        const ok = await _typeVerified('Medflow Login', '~userInput', value) || await _typeScopedOrCom('Medflow Login', '~userInput', value);
        if (!ok) {
            console.warn('[type2] scoped sendKeys failed — falling back to OS-level typing');
            osActivate('Medflow Login', _hwndCache['Medflow Login']);
            osType(value);
        }
    }

    async click3() {
        await _clickScoped('Medflow Login', '~passInput', false, 50004);
    }

    async type4(value) {
        const ok = await _typeVerified('Medflow Login', '~passInput', value) || await _typeScopedOrCom('Medflow Login', '~passInput', value);
        if (!ok) {
            console.warn('[type4] scoped sendKeys failed — falling back to OS-level typing');
            osActivate('Medflow Login', _hwndCache['Medflow Login']);
            osType(value);
        }
    }
    ...
    async click5() {
        await _clickScoped('Medflow Login', '~loginBtn', false, 50000);
    }


    // ════════════════════════════════════════════════════════════
    // [W2] Medflow Clinical System (new window)
    // ════════════════════════════════════════════════════════════
    async click6() {
        await _clickScoped('Medflow Clinical System', '~settingsBtn', false, 50000);
    }


    // ════════════════════════════════════════════════════════════
    // [W3] Medflow Settings (new window)
    // ════════════════════════════════════════════════════════════
    async click7() {
        osScopedInvoke(_liveHwnd("Medflow Settings", _hwndCache[_mainTitleFrag]), {"automationId":"notifyChk","className":"","name":"Receive notifications"}, null, null, null, "Medflow Settings", false, true);
    }

    async click8() {
        await _clickScoped('Medflow Settings', '~themeDark', false, 50013);
    }

    async click9() {
        await _clickScoped('Medflow Settings', '~okBtn', false, 50000);
    }


    // ════════════════════════════════════════════════════════════
    // [W4] Medflow Clinical System (new window)
    // ════════════════════════════════════════════════════════════
    async click10() {
        await _clickScoped('Medflow Clinical System', '~btn_patient_data', false, 50000);
    }

    async click11() {
        await _clickScoped('Medflow Clinical System', '//Text[@Name="63980186"]', true, 50020);
    }


    // ════════════════════════════════════════════════════════════
    // [W5] Medflow Patient Detail (new window)
    // ════════════════════════════════════════════════════════════
    async click12() {
        await _clickScoped('Medflow Patient Detail', '~closeBtn', false, 50000);
    }
}
...
async function run() {
    ...
    _dialogRects = {"Medflow Login":{"left":64,"top":64,"width":1440,"height":740},"Medflow Clinical System":{"left":-9,"top":-9,"width":1938,"height":1038},"Medflow Settings":{"left":128,"top":128,"width":1440,"height":740}};
    await ensureAppium();
    ...
    console.log(`[session] Root session ${_rootSid} ready`);
        ...
        const page = new MockGoldenMedflowPageByClass();

    // ════════════════════════════════════════════════════════════
    // [W1] Medflow Login (main window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: Medflow Login', async () => { await _switchWindow('Medflow Login'); osActivate('Medflow Login', _hwndCache['Medflow Login']); });
            await _step('1:click User ID', () => page.click1());
            await _step('2:type admin', () => page.type2('admin'));
            await _step('3:click Password', () => page.click3());
            await _step('4:type 1234', () => page.type4('1234'));
            await _step('5:click LOGIN', () => page.click5());
    ...
    // ════════════════════════════════════════════════════════════
            ...
            await _step('6:click SETTINGS', () => page.click6());

    // ════════════════════════════════════════════════════════════
    // [W3] Medflow Settings (new window)
    // ════════════════════════════════════════════════════════════
            await _step('7:click Receive notifications', () => page.click7());
            await _step('8:click Dark', () => page.click8());
            await _step('9:click OK', () => page.click9());

    // ════════════════════════════════════════════════════════════
    // [W4] Medflow Clinical System (new window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: Medflow Clinical System', async () => { await _switchWindow('Medflow Clinical System'); osActivate('Medflow Clinical System', _hwndCache['Medflow Clinical System']); });
            await _step('10:click PATIENT DATA', () => page.click10());
            await _step('11:doubleClick 63980186', () => page.click11());

    // ════════════════════════════════════════════════════════════
    // [W5] Medflow Patient Detail (new window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: Medflow Patient Detail', async () => { await _switchWindow('Medflow Patient Detail'); osActivate('Medflow Patient Detail', _hwndCache['Medflow Patient Detail']); });
            await _step('12:click CLOSE', () => page.click12());
    } finally {
