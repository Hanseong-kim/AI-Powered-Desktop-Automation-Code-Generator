...
//   [W3] "HeidiSQL 업데이트 확인 ..." (opened during recording)

class MockGoldenHeidiSQLPageByClass {

    // ════════════════════════════════════════════════════════════
    // [W1] HeidiSQL 12.20.0.7320 - 세션 관리자 (main window)
    // ════════════════════════════════════════════════════════════
    async click1() {
        osExpandCollapse(_liveHwnd("HeidiSQL 12.20.0.7320 - 세션 관리자", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":""}, null, 0, 6);
    }

    async click2() {
        osScopedInvoke(_liveHwnd("HeidiSQL 12.20.0.7320 - 세션 관리자", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"로그 기록"}, null, 60, null, "환경 설정");
    }

    async click3() {
        osScopedInvoke(_liveHwnd("HeidiSQL 12.20.0.7320 - 세션 관리자", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"SQL"}, null, 61, null, "환경 설정");
    }

    async click4() {
        osScopedInvoke(_liveHwnd("HeidiSQL 12.20.0.7320 - 세션 관리자", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"그리드 서식 설정"}, null, 61, null, "환경 설정");
    }
    ...
    // ════════════════════════════════════════════════════════════
    async click6() {
        osExpandCollapse(_liveHwnd("HeidiSQL 12.20.0.7320 - 세션 관리자", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"TButton","name":"더 보기"}, null, 1, 6);
    }


    // ════════════════════════════════════════════════════════════
    // [W3] HeidiSQL 업데이트 확인 ... (new window)
    // ════════════════════════════════════════════════════════════
    async click7() {
        await _clickScoped('HeidiSQL 업데이트 확인 ...', '//Button[@ClassName="TButton" and @Name="취소"]', false, 0);
    }
}
...
async function run() {
    ...
    _dialogRects = {"HeidiSQL 12.20.0.7320 - 세션 관리자":{"left":797,"top":144,"width":875,"height":612},"HeidiSQL 업데이트 확인 ...":{"left":399,"top":103,"width":500,"height":575}};
    await ensureAppium();
    _rootSid = await _createSession('Root');
    console.log(`[session] Root session ${_rootSid} ready`);
        await launchApp("C:\\Program Files\\HeidiSQL\\heidisql.exe", [], "HeidiSQL 12.20.0.7320 - 세션 관리자", {"left":797,"top":144,"width":875,"height":612});

        const page = new MockGoldenHeidiSQLPageByClass();

    // ════════════════════════════════════════════════════════════
    // [W1] HeidiSQL 12.20.0.7320 - 세션 관리자 (main window)
    // ════════════════════════════════════════════════════════════
            await _step('1:select item #0 (unnamed)', () => page.click1());
            await _step('2:click 로그 기록 (cross-window)', () => page.click2());
            await _step('3:click SQL (cross-window)', () => page.click3());
            await _step('4:click 그리드 서식 설정 (cross-window)', () => page.click4());
            // [STEP 5] click: no selector/anchor captured — coordinate replay is forbidden (2026-07-10)
            _failures.push('5:click:no-selector');

    // ════════════════════════════════════════════════════════════
    // [W2] HeidiSQL 12.20.0.7320 - 세션 관리자 (new window)
    // ════════════════════════════════════════════════════════════
            await _step('6:select item #1 (unnamed)', () => page.click6());

    // ════════════════════════════════════════════════════════════
    // [W3] HeidiSQL 업데이트 확인 ... (new window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: HeidiSQL 업데이트 확인 ...', async () => { await _switchWindow('HeidiSQL 업데이트 확인 ...'); osActivate('HeidiSQL 업데이트 확인 ...', _hwndCache['HeidiSQL 업데이트 확인 ...']); });
            await _step('7:click 취소', () => page.click7());
    } finally {
