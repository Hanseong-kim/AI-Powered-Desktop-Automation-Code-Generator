...
class MockGoldenFileZillaPageByClass {

    // ════════════════════════════════════════════════════════════
    // [W1] FileZilla (main window)
    // ════════════════════════════════════════════════════════════
    async click1() {
        osExpandCollapse(_liveHwnd("FileZilla", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"파일(F)"}, "사이트 관리자(S)...\tCtrl+S", 0, 8);
    }
    ...
    // ════════════════════════════════════════════════════════════
    async click2() {
        await _clickScoped('사이트 관리자', '//Button[@ClassName="Button" and @Name="새 폴더(F)"]', false, 0);
    }
    ...
    async click4() {
        await _clickScoped('사이트 관리자', '//Tree[@ClassName="SysTreeView32" and @Name="항목 선택(S):"]', false, 0);
    }

    async click5() {
        await _clickScoped('사이트 관리자', '//Button[@ClassName="Button" and @Name="새 사이트(N)"]', false, 0);
    }

    async type6(value) {
        const hs = _listWindowHwnds('사이트 관리자');
        if (!hs.length) {
            console.warn('[type6] window "사이트 관리자" not found by EnumWindows — activation/typing may miss');
        }
        osActivate('사이트 관리자', hs[0]);
        osType(value);
    }
    ...
    async click8() {
        osScopedInvoke(_liveHwnd("사이트 관리자", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"고급"}, null, 45, null, "사이트 관리자");
    }

    async click9() {
        await _clickScoped('사이트 관리자', '//Edit[@ClassName="Edit" and @Name="기본 로컬 디렉터리(L):"]', false, 0);
    }

    async type10(value) {
        const hs = _listWindowHwnds('사이트 관리자');
        if (!hs.length) {
            console.warn('[type10] window "사이트 관리자" not found by EnumWindows — activation/typing may miss');
        }
        osActivate('사이트 관리자', hs[0]);
        osType(value);
    }
    ...
    async click11() {
        await _clickScoped('사이트 관리자', '//Edit[@ClassName="Edit" and @Name="기본 원격 디렉터리(E):"]', false, 0);
    }

    async type12(value) {
        const hs = _listWindowHwnds('사이트 관리자');
        if (!hs.length) {
            console.warn('[type12] window "사이트 관리자" not found by EnumWindows — activation/typing may miss');
        }
        osActivate('사이트 관리자', hs[0]);
        osType(value);
    }
    ...
    async click13() {
        await _clickScoped('사이트 관리자', '~-31777', false, 0);
    }
    ...
    async click15() {
        await _clickScoped('사이트 관리자', '//Button[@ClassName="Button" and @Name="취소"]', false, 0);
    }

    async click16() {
        let _expHs16 = _listWindowHwnds('사이트 관리자');
        if (!_expHs16.length) {
            const _expTail16 = '사이트 관리자'.split(' - ').pop();
            if (_expTail16 !== '사이트 관리자') _expHs16 = _listWindowHwnds(_expTail16);
        }
        const _expHwnd16 = _expHs16.find(h => h === osForegroundHwnd()) || _expHs16[0] || _liveHwnd("사이트 관리자", _hwndCache[_mainTitleFrag]);
        osExpandCollapse(_expHwnd16, {"automationId":"","className":"","name":"편집(E)"}, null, null, null);
    }

    async click17() {
        osScopedInvoke(_liveHwnd("사이트 관리자", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":""}, null, null, null, "Desktop Automation Code Generator - Chrome", false, true);
    }
    ...
    // ════════════════════════════════════════════════════════════
    async click18() {
        await _clickScoped('FileZilla', '~-31824', false, 0);
    }
}
...
async function run() {
    ...
    _dialogRects = {"FileZilla":{"left":2502,"top":93,"width":960,"height":758},"사이트 관리자":{"left":2509,"top":216,"width":946,"height":511}};
    await ensureAppium();
    _rootSid = await _createSession('Root');
    console.log(`[session] Root session ${_rootSid} ready`);
        await launchApp("C:\\Program Files\\FileZilla FTP Client\\filezilla.exe", [], "FileZilla", {"left":2502,"top":93,"width":960,"height":758});

        const page = new MockGoldenFileZillaPageByClass();
    ...
    // ════════════════════════════════════════════════════════════
            ...
            await _step('8:click 고급 (cross-window)', () => page.click8());
            await _step('9:click 기본 로컬 디렉터리(L):', () => page.click9());
            await _step('10:type asdf', () => page.type10('asdf'));
            await _step('11:click 기본 원격 디렉터리(E):', () => page.click11());
            await _step('12:type asdf', () => page.type12('asdf'));
            await _step('13:click panel', () => page.click13());
            await _step('14:click panel', () => page.click14());
            await _step('15:click 취소', () => page.click15());
            await _step('16:expandCollapse 편집(E)', () => page.click16());
