...
//   [W4] "TeamViewer" (opened during recording)

class MockGoldenTeamViewerPageByClass {

    // ════════════════════════════════════════════════════════════
    // [W1] TeamViewer (main window)
    // ════════════════════════════════════════════════════════════
    async click1() {
        osScopedInvoke(_liveHwnd("TeamViewer", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"ID를 복사하세요"});
    }

    async click2() {
        osScopedInvoke(_liveHwnd("TeamViewer", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"비밀번호를 복사하세요"});
    }

    async click3() {
        osScopedInvoke(_liveHwnd("TeamViewer", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"비밀번호를 새로고침하세요"});
    }

    async click4() {
        osScopedInvoke(_liveHwnd("TeamViewer", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"세션 코드"});
    }
    ...
    async click6() {
        osScopedInvoke(_liveHwnd("TeamViewer", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"Windows와 함께 TeamViewer 시작"});
    }
    ...
    // ════════════════════════════════════════════════════════════
    async click8() {
        await _clickScoped('빠른 연결 허용', '//Text[@Name="이 장치에 Easy Access 권한 부여"]', false, 0);
    }
    ...
    // ════════════════════════════════════════════════════════════
    async click9() {
        await _clickScoped('빠른 연결 허용', '//Edit[@ClassName="Edit" and @Name="이메일"]', false, 0);
    }

    async type10(value) {
        await _typeScopedOrCom('빠른 연결 허용', '//Edit[@ClassName="Edit" and @Name="이메일"]', value);
    }

    async click11() {
        await _clickScoped('빠른 연결 허용', '//Edit[@ClassName="Edit" and @Name="비밀번호"]', false, 0);
    }

    async type12(value) {
        await _typeScopedOrCom('빠른 연결 허용', '//Edit[@ClassName="Edit" and @Name="비밀번호"]', value);
    }

    async click13() {
        await _clickScoped('빠른 연결 허용', '//Window[@ClassName="#32770" and @Name="빠른 연결 허용"]', false, 0);
    }

    async click14() {
        await _clickScoped('빠른 연결 허용', '//Button[@ClassName="Button" and @Name="취소"]', false, 0);
    }

    async click16() {
        osScopedInvoke(_liveHwnd("TeamViewer", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"TeamViewer에 로그인"}, null, 587, null, "TeamViewer");
    }

    async click17() {
        osScopedInvoke(_liveHwnd("TeamViewer", _hwndCache[_mainTitleFrag]), {"automationId":"email-field","className":"","name":""}, null, 281, null, "TeamViewer");
    }

    async type18(value) {
        const hs = _listWindowHwnds('TeamViewer');
        if (!hs.length) {
            console.warn('[type18] window "TeamViewer" not found by EnumWindows — activation/typing may miss');
        }
        osActivate('TeamViewer', hs[0]);
        osType(value);
    }

    async click21() {
        osScopedInvoke(_liveHwnd("TeamViewer", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"TeamViewer에 로그인"}, null, 610, null, "TeamViewer");
    }

    async click22() {
        osScopedInvoke(_liveHwnd("TeamViewer", _hwndCache[_mainTitleFrag]), {"automationId":"email-field","className":"","name":""}, null, 286, null, "TeamViewer");
    }

    async type23(value) {
        const hs = _listWindowHwnds('TeamViewer');
        if (!hs.length) {
            console.warn('[type23] window "TeamViewer" not found by EnumWindows — activation/typing may miss');
        }
        osActivate('TeamViewer', hs[0]);
        osType(value);
    }
    ...
    async click24() {
        osScopedInvoke(_liveHwnd("TeamViewer", _hwndCache[_mainTitleFrag]), {"automationId":"","className":"","name":"계속"}, null, 353, null, "TeamViewer");
    }
}
...
async function run() {
    ...
    _dialogRects = {"TeamViewer":{"left":337,"top":48,"width":1300,"height":977},"빠른 연결 허용":{"left":337,"top":48,"width":1300,"height":977}};
    await ensureAppium();
    ...
    console.log(`[session] Root session ${_rootSid} ready`);
        ...
        const page = new MockGoldenTeamViewerPageByClass();

    // ════════════════════════════════════════════════════════════
    // [W1] TeamViewer (main window)
    // ════════════════════════════════════════════════════════════
            await _step('1:click ID를 복사하세요', () => page.click1());
            ...
            await _step('5:type 123456789', () => page.type5('123456789'));
    ...
    // [W2] 빠른 연결 허용 (new window)
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: 빠른 연결 허용', async () => { await _switchWindow('빠른 연결 허용'); osActivate('빠른 연결 허용', _hwndCache['빠른 연결 허용']); });
    ...
    // ════════════════════════════════════════════════════════════
            await _step('switch to window: 빠른 연결 허용', async () => { await _switchWindow('빠른 연결 허용'); osActivate('빠른 연결 허용', _hwndCache['빠른 연결 허용']); });
            await _step('9:click 이메일', () => page.click9());
            await _step('10:type asdf', () => page.type10('asdf'));
            await _step('11:click 비밀번호', () => page.click11());
            await _step('12:type asdf', () => page.type12('asdf'));
            await _step('13:click 빠른 연결 허용', () => page.click13());
            await _step('14:click 취소', () => page.click14());

    // ════════════════════════════════════════════════════════════
    // [W4] TeamViewer (new window)
    // ════════════════════════════════════════════════════════════
            // [STEP 15] drag scope-out — replay skipped (event scope: Click/Type/DoubleClick/Scroll, 2026-07-10)
            await _step('16:click TeamViewer에 로그인 (cross-window)', () => page.click16());
            await _step('17:click 이메일 (cross-window)', () => page.click17());
            await _step('18:type asdf', () => page.type18('asdf'));
            // [STEP 19] click: no selector/anchor captured — coordinate replay is forbidden (2026-07-10)
            _failures.push('19:click:no-selector');
            // [STEP 20] click: no selector/anchor captured — coordinate replay is forbidden (2026-07-10)
            _failures.push('20:click:no-selector');
            await _step('21:click TeamViewer에 로그인 (cross-window)', () => page.click21());
            await _step('22:click 이메일 (cross-window)', () => page.click22());
            await _step('23:type asdf', () => page.type23('asdf'));
            await _step('24:click 계속 (cross-window)', () => page.click24());
            // [STEP 25] click: no selector/anchor captured — coordinate replay is forbidden (2026-07-10)
            _failures.push('25:click:no-selector');
    } finally {
